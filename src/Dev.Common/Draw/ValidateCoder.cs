...
namespace Dev.Common.Draw
{
    public class ValidateCoder
    {
        ...
        private string GetRandomLetter(int length)
        {
        ...
        /// </summary>
        /// <returns></returns>
        private string GetRandomAddNum()
        {
            int num1 = int.Parse(GetRandomNums(1));
            int num2 = int.Parse(GetRandomNums(1));
            AddResult = (num1 + num2).ToString();
        ...
        public Bitmap CreateImage(string code, ValidateCodeType codeType, int imageWidth)
        {
            ...
            float scale = 120.0f / (float)imageWidth;
            ...
            for (int i = 0; i < code.Length; i++)
            {
                ...
                Font font = new Font(fontName, FontWidth, FontStyle.Regular);
                // "+"符号不做倾斜处理
        ...
        /// 获取指定长度的验证码图片
        /// </summary>
        public Bitmap CreateImage(int length, out string code, int imageWidth, ValidateCodeType codeType = ValidateCodeType.NumberAndLetter)
        {
            if (length <= 0)
            {
                code = null;
